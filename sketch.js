...
function preload(){
  ...
  sound1=loadSound("sounds/biowastecollect.wav");
  gamend=loadSound("sounds/gamend.wav")
  sound2=loadSound("sounds/nbiowastecollect.wav");
}
...
function draw(){
        ...
        if(gameState==="PLAY"){
                ...
                stroke("black")
                strokeWeight(3);
                text("Use your arrow keys to move the car",420,300);
                text("Collect biodegradable wastes and avoid non-biodegradable wastes",200,350);
...
function spawnBiodegradableWaste() {
  if(frameCount % 350 === 0) {
    ...
    bioWaste.velocityX=-(random(2,4)+3*score/10);
    //bioWaste.debug=true;
...
function spawnNonBiodegradableWaste() {
  if(frameCount % 300 === 0) {
    ...
    nBioWaste.velocityX=-(random(2,4)+3*score/10);
